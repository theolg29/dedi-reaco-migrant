...
|---|---|---|
| [`bureau-et-salles.md`](./bureau-et-salles.md) | Gestion d'une salle (avatar, dialogue, porte, imprimante) | `BureauManager`, `PrinterAnimation` |
...
| [`divers.md`](./divers.md) | Utilitaires (surbrillance, fondu, intro, locomotion) | `XRHoverHighlight`, `ButtonBIndicator`, `HighlightSettings`, `FadeManager`, `IntroManager`, `DevLocomotion`, `CanvasFaceCamera`, `GameObjectSwapper` |
...
| `PrinterAnimation.cs` | Anime l'imprimante (aspiration, impression, sortie de feuille, ou bourrage papier) — purement décoratif |
...
| `GameObjectSwapper.cs` | Désactive un GameObject et active un autre (ex: changement d'éclairage) |
| `MainMenuManager.cs` | Menu d'accueil : charge la scène suivante à la pression de la gâchette (peu importe la direction visée) |
